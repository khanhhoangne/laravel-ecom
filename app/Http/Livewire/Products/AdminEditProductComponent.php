...
class AdminEditProductComponent extends Component
{
	...
	public function setRules() {
        return [
			...
			'product_slug' => 'required|unique:shop_products,product_slug,' . $this->productId,
			'product_code' => 'unique:shop_products,product_code,' . $this->productId
        ];
    }
	...
	public function mount($product_slug) {
		...
		$this->is_continued = $product->is_continued;
		...
		// Get images
		$productGallery = ProductGallery::where('product_id', $this->productId)->get();
		...
		// Get option
		$productOption = ProductOption::where('product_id', $this->productId)->get();

		foreach ($productOption as $key => $value) {
			$option = $value->option;

			$this->productOptionDetail[$option][] = $value->detail;
		}

		// Get Price
		$productPrice = ProductPrice::where('product_id', $this->productId)->get();

		foreach ($productPrice as $key => $value) {
			$this->optionPriceDetail[] = $value->price;
		}
    }
    ...
    public function updateProduct() {
        ...
        if (!empty($this->newImage)) {
            $imageName = $this->product_slug . Carbon::now()->timestamp . '.' . $this->newImage->extension();
            $this->newImage->storeAs('public/uploads/products', $imageName);
		...
		if (count($this->newImages)) {
            $productGallery = ProductGallery::where('product_id', $productId)->get(['id']);
		    ProductGallery::destroy($productGallery->toArray());
		...
		$productOption = ProductOption::where('product_id', $productId)->get(['id']);
		ProductOption::destroy($productOption->toArray());
		
		$productPrice = ProductPrice::where('product_id', $productId)->get(['id']);
		ProductPrice::destroy($productPrice->toArray());
